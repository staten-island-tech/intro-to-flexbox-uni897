...
for(let publisher in publishers){
    ...
    publishDiv.insertAdjacentHTML("afterbegin", `
        ...
        <div class="amount">${publishers[publisher]}</div>
        `)
}
videoGames.filter(game => game.ratings.metacritic > 95).forEach(game =>{
    document.querySelector(".games").insertAdjacentHTML("afterbegin", `<h2>${game.title}</h2>`)
})